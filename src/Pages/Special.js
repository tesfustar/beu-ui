...
const Special = () => {
 ...
 const handleSubmit=(event,item)=>{
  ...
  return (
    <>
      {isLoading ?  (<Skeleton />) :(
       <div className='grid grid-flow-row-dense grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6'>
          {foods?.map((product)=>(
              <div className='relative flex flex-col m-1  cursor-pointer ' key={product.id}>
                <div onClick={()=>openModal(product)}
                className='flex items-center justify-center cursor-pointer '>
                      
              <img src={product.img} alt="" className='rounded-xl w-full h-40 object-cover'
           height={300}/>
              </div>
              ...
              <div onClick={()=>addToCart(product)}
               className='justify-end bg-gradient-to-r from-red-600 to-orange-400 p-1 rounded-xl border border-white cursor-pointer '>
    ...
    <AiOutlinePlus size={20} className='text-white  cursor-pointer '/>
              ...
              <h1 className='font-bold text-lg text-gray-400 '>BIRR {product.price} </h1>
